...
def get_svo(sentence, subject):
    ...
    phrase : list of token, tag pairs for that lie within the indexes of
                the variables above
    """
    subject_idx = (i for i, v in enumerate(sentence)
                    if v[0].lower() == subject), next(None)
...
def get_subject(fulltext):
    document = fulltext
    document = clean_document(fulltext)
    #document = tokenize_sentences(document)
    #document = get_entities(document)
    subject = extract_subject(document)
    return subject
# if __name__ == '__main__':
#     url = 'https://www.washingtonpost.com/politics/democrats-signal-aggressive-investigations-of-trump-while-resisting-impeachment-calls/2018/11/11/63945180-e5d4-11e8-bbdb-72fdbf9d4fed_story.html'
